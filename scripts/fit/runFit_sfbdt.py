...
parser.add_argument('--mode', default=None, choices=['main', 'fit_var_rwgt'], help='fit schemes in controlling the nuiences.')
...
parser.add_argument('--automc-threshold', default=20, help='Set the threshold for autoMCStat')
...
syst_list = ['lumi_13TeV', 'pileup', 'l1PreFiring', 'psWeightIsr', 'psWeightFsr', 'JES_Total', 'JER', 'frac_bb', 'frac_cc', 'frac_l', 'QCDFlvComposRealMG']
if args.mode == 'fit_var_rwgt':
...
for bin in bins:
    cb.cp().bin([bin]).ExtractShapes(
        os.path.join(args.inputdir, 'inputs_mcDataRwgtUnce_%s.root' % bin),
        '$PROCESS',
        '$PROCESS_$SYSTEMATIC'
        )
...
with open(os.path.join(args.workdir, outputname), 'w') as fout:
    ...
    if useAutoMCStats:
        fout.write('* autoMCStats {automc_threshold} 1\n'.format(automc_threshold=args.automc_threshold))
...
if args.run_full_unce_breakdown and args.mode == 'main':
    cmd = '''
cd {workdir} && \
combine -M MultiDimFit -m 125 SF.root --algo=grid --robustFit=1 --points=50 -n Grid --redefineSignalPOIs SF_{flv_poi1} {ext_fit_options} && \
combine -M MultiDimFit -m 125 SF.root --algo=singles --robustFit=1 -n Bestfit --saveWorkspace {ext_fit_options} &&
'''.format(workdir=args.workdir, flv_poi1=flv_poi1, ext_fit_options=ext_fit_options)
    comb_plot_opt = []
    syst_list_iter = [s for s in syst_list[::-1]]
...
if args.run_full_unce_breakdown and args.mode == 'fit_var_rwgt':
    cmd = '''
cd {workdir} && \
combine -M MultiDimFit -m 125 SF.root --algo=grid --robustFit=1 --points=50 -n Grid --redefineSignalPOIs SF_{flv_poi1} {ext_fit_options} && \
combine -M MultiDimFit -m 125 SF.root --algo=singles --robustFit=1 -n Bestfit --saveWorkspace {ext_fit_options} &&
'''.format(workdir=args.workdir, flv_poi1=flv_poi1, ext_fit_options=ext_fit_options)
    comb_plot_opt = []
    syst_list_iter = [s for s in syst_list[::-1] if s not in ['fitVarRwgt']]
    for i in range(len(syst_list_iter)):
        params = ','.join(syst_list_iter[:i+1])+',fitVarRwgt'
        suffix = 'freeze_till_{syst}'.format(syst=syst_list_iter[i])
        cmd += 'combine -M MultiDimFit -m 125 --algo=grid --points=50 higgsCombineBestfit.MultiDimFit.mH125.root --redefineSignalPOIs SF_{flv_poi1} --snapshotName MultiDimFit --setParameters fitVarRwgt=1 --freezeParameters {params} -n {suffix} && \n'.format(
            flv_poi1=flv_poi1, params=params, suffix=suffix
        )
        comb_plot_opt.append('higgsCombine{suffix}.MultiDimFit.mH125.root:{suffix}:{i}'.format(suffix=suffix, i=i+2))
    cmd += 'plot1DScanWithOutput.py higgsCombineGrid.MultiDimFit.mH125.root --others {others_params} --POI SF_{flv_poi1} -o full_unce_breakdown --breakdown {breakdown}'.format(
