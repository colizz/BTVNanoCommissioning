...
for year, file in datasets_skimmed_files.items():
  dataset_skimmed = json.load(open(datasets_skimmed[args.year], 'r'))
  genweights_dict = load(genweights_files[args.year])['sum_genweights']
  xsec_dict = {f"{k}_{args.year}" : d["files"][0]["metadata"]["xsec"] for k, d in json.load(open(xsec_file, 'r')).items() if not k.startswith("DATA")}
  ...
  for k, d in dataset_QCD.items():
      sample = k.split(f"_{args.year}")[0]
  ...
  output_dict = {k : d for k,d in dataset_DATA.items()}
  for d in [dataset_QCD, dataset_VJets, dataset_ggH]:
    output_dict.update(d)

  outfilename = f"skimmed_dataset_definition_{args.year}_with_metadata.json"
